...
function applyDocTime(url, args) {
  ...
  if (start && end && start > end) throw new TypeError('startDateTime must be before endDateTime.');
...
async function fetchJson(url, fetchImpl = globalThis.fetch) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (declaredLength > MAX_RESPONSE_BYTES) {
        throw new GdeltApiError('GDELT response exceeded the safe size limit.', { status: response.status, url: String(url) });
      }
  ...
  throw new GdeltApiError(`GDELT request failed: ${lastError instanceof Error ? lastError.message : String(lastError)}`, { url: String(url) });
}
